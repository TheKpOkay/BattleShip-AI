...
public class ComputerPlayer extends Player
{
    ...
    public boolean attack(Player enemy, Location loc)
    {
        Random ran = new Random();
            ...
            while (super.getGuessBoard()[row][col] != 0) //checks to see if the guess has not been guessed yet
            {
                row = ran.nextInt(10);
                col = ran.nextInt(10);
            }

            Location loa = new Location(row, col);

            if (enemy.hasShipAtLocation(loa))
            {
                super.getGuessBoard()[row][col] = 1;
                enemy.getShip(loa).takeHit(loa);
                if (enemy.getShip(loa).isSunk())
                {
                    enemy.removeShip(getShip(loa));
                    return true;
                }
            }
            else
